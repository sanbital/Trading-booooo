...
live_guards = "supabase/functions/market-autotrader/live-guards.ts"
replace_once(
    live_guards,
...
test = "supabase/functions/market-autotrader/live-guards.test.ts"
Path(test).write_text('''import { assertEquals } from "https://deno.land/std@0.224.0/assert/mod.ts";
...
migration = Path("supabase/migrations/20260813081500_pre_t1_positive_net_exit_guard_v768.sql")
migration.write_text(r'''-- v7.6.8: pre-T1 profit protection must never manufacture a loss exit.
--
...
print("v7.6.8 pre-T1 positive-net guard patch staged")
