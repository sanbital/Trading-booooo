...
-- Applied to production through Supabase migration API. Kept outside supabase/migrations
-- because the legacy broad migration workflow mutates unrelated Edge secrets/config.
--
-- Scope:
...
--   * pre-T1 only
--
-- The floor is derived from actual applied BUY principal/fee, remaining quantity,
-- observed fee rate + 1bp exit-fee safety, and the quantity-aware executable bid
-- already persisted by the monitor. Existing later profit-lock/trailing stages,
-- sell reason codes, 3m Recovery, Late Recovery, and TTL ownership remain unchanged.
...
begin
  ...
  -- Dynamic economic breakeven arm. Current executable price is the full-position,
  -- quantity-aware bid VWAP written by the monitor in this same UPDATE.
  ...
  -- Preserve the existing higher-profit ratchets unchanged.
  ...
  -- The existing v7.6.8 sanitizer rejects non-positive newly armed floors. Use exact
  -- economic net for dynamic protection; keep legacy estimation for legacy stages.
  ...
  if v_candidate_stop > coalesce(new.trailing_stop, 0) then
    ...
    new.metadata := coalesce(new.metadata, '{}'::jsonb) || jsonb_build_object(
      ...
      jsonb_build_object(
        ...
        'dynamic_breakeven_price', case when v_dynamic_be_price > 0 then v_dynamic_be_price else null end,
        'current_executable_price', case when v_current_exec > 0 then v_current_exec else null end,
        'current_executable_net_profit_quote', case when v_current_exec > 0 then v_current_exact_net else null end,
        'exact_buy_principal_quote', case when v_buy_principal > 0 then v_buy_principal else null end,
        'exact_buy_fee_quote', case when v_exact_buy_fee > 0 then v_exact_buy_fee else null end,
        'observed_buy_fee_bps', round(v_observed_entry_fee_bps, 4),
        'dynamic_exit_fee_bps', case when v_exit_fee_rate > 0 then round(v_exit_fee_rate * 10000, 4) else null end,
        'exact_unrecovered_cost_quote', case when v_unrecovered_cost > 0 then v_unrecovered_cost else null end,
        'dynamic_floor_net_bps', case when v_dynamic_armed then round(v_dynamic_locked_net_bps, 4) else null end,
...
-- Fail closed if the function or the downstream positive-net sanitizer is missing.
do $$
declare
  v_def text;
  v_sanitizer text;
...
begin
  select pg_get_functiondef('public.protect_lob_open_profit_v6130()'::regprocedure) into v_def;
