...
import {exitDecision,entryDecision,VARIANTS} from './candidates.mjs';
...
function simulate(r,variant,e,boundary,prior=false){
 const gate=prior||variant==='BASELINE'?{verdict:'KEEP'}:entryDecision(r.features,r.features.signal5Close,r.features.referenceClose,variant);
 ...
 for(const b of usable){
  ...
  const d=prior?nextExit48(state,open,t,variant,bars):exitDecision(state,open,t,variant,bars);
...
for(const [scenario,e] of Object.entries(protocol.execution_scenarios))for(const boundary of protocol.bar_order_bounds){
 ...
 for(const variant of EXIT48_VARIANTS.filter(v=>v!=='BASELINE')){
  const xs=rows.map(r=>simulate(r,variant,e,boundary,true));
  for(const [cohort,filter] of Object.entries(groups))priorResults.push({scenario,boundary,variant,cohort,...stats(xs.filter(filter)),delta:stats(xs.filter(filter)).net-stats(base.filter(filter)).net});
 }
}
...
let seed=20260911;const random=()=>{seed=(Math.imul(1664525,seed)+1013904223)>>>0;return seed/4294967296;};
const bootstrap=[];
for(const variant of VARIANTS.slice(1)){
 const xs=runs[`normal/SKIP_ENTRY_BAR/${variant}`],deltas=xs.map((x,i)=>x.net-normal[i].net),samples=[];
 for(let i=0;i<10000;i++){let s=0,n=0;while(n<deltas.length){const start=Math.floor(random()*deltas.length);for(let j=0;j<5&&n<deltas.length;j++,n++)s+=deltas[(start+j)%deltas.length];}samples.push(s);}
 const tail=.01/(2*(VARIANTS.length-1));
 samples.sort((a,b)=>a-b);bootstrap.push({variant,totalDelta:sum(deltas,x=>x),familywiseLower99:samples[Math.floor(samples.length*tail)],familywiseUpper99:samples[Math.ceil(samples.length*(1-tail))-1],correction:'BONFERRONI_7_FROZEN_COMPARISONS',blockTrades:5,resamples:10000,scope:'DESCRIPTIVE_SEEN_DATA_NOT_PROMOTION_EVIDENCE'});
}
const outcome={protocol,results,priorResults,runs,fidelity,recordedDecisionParity:parity,paritySummary,bootstrap,livePromotion:false,
 blockingGates:['Only 3 trades after latest code deployment versus required 100','No independent validation trades','Funding unverified','Historical cash/slots and replacement signals not reconstructed','Native intraminute execution fidelity incomplete'],
