...
const seedPath=process.env.SEED_FUNDING_CACHE||"";
const seed=seedPath?JSON.parse(readFileSync(seedPath,"utf8")):{coverage:[]};
const reusable=new Map((seed.coverage||[])
  .filter(x=>Number(x.startTime)<=START&&Number(x.endTime)>=END)
  .map(x=>[x.symbol,{symbol:x.symbol,startTime:START,endTime:END,events:(x.events||[]).filter(row=>Number(row.fundingTime)>=START&&Number(row.fundingTime)<=END)}]));
...
const missing=symbols.filter(symbol=>!reusable.has(symbol));
for(const symbol of symbols)if(reusable.has(symbol))coverage.push(reusable.get(symbol));
async function worker(){
  while(next<missing.length){const symbol=missing[next++];coverage.push(await fetchSymbol(symbol));done++;if(done%10===0)console.log("FUNDING_CACHE",done,"/",missing.length);}
}
...
console.log("FUNDING_CACHE_COMPLETE",coverage.length,"REUSED",symbols.length-missing.length,"FETCHED",missing.length);
