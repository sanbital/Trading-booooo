...
## 11. 수정 1 (2026-09-24 09:49Z, 성숙한 outcome 0건 상태에서 기록)

- **판정 표본 시작 = 2026-09-25 KST 일(2026-09-24 15:00Z) 이후의 첫 완전 KST 일.** 배포일(KST 2026-09-24) 행은 배관·비용 점검에만 쓴다.
- 이유(결과와 무관한 데이터 유효성): shadow 는 당일 순위 이력을 자기 이전 사이클로만 만든다. 배포 시점 이전 이력이 없으므로
  배포일에는 `first_top3_today`, `first_top10_today`, `minutes_in_top10_today`, `EXTENDED_LEADER` 가 과대/과소 판정된다
  (첫 사이클에서 하루 종일 1위였던 NOMUSDT 가 "당일 Top3 첫 진입"으로 선정됨). velocity 는 15/60분 이력이 쌓인 뒤부터 유효하다.
- 임계값·lane·비용·horizon 은 바꾸지 않는다. 같은 이유로 장애 후 재개된 날도 그 KST 일은 판정 표본에서 제외한다.
