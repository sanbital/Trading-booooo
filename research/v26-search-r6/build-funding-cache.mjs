...
async function fetchSymbol(symbol){
  ...
  let last;
  for(let attempt=1;attempt<=6;attempt++)try{
    const response=await fetch(url,{signal:AbortSignal.timeout(30_000)});
    if(!response.ok)throw Error(`HTTP_${response.status}`);
    const rows=await response.json();
    if(!Array.isArray(rows)||rows.some(row=>row.symbol!==symbol||!Number.isSafeInteger(Number(row.fundingTime))||
      !Number.isFinite(Number(row.fundingRate))||!(Number(row.markPrice)>0)))throw Error("INVALID_ROWS");
