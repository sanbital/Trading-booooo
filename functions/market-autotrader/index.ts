...
async function p10ScanCycle(cycleId: string, settings: TradingSettings & JsonRecord) {
  ...
  let explicitManualFutures: FuturesExposure[] = [];
  try {
    const manualRows = await db(
      "trading_asset_locks?exchange=eq.binance_futures&state=eq.LOCKED&select=asset,metadata",
    ) as any[];
    explicitManualFutures = (manualRows || [])
      .filter((row) => row?.metadata?.v17ManualPosition === true)
      .map((row) => ({
        market: `${String(row.asset || "").toUpperCase()}USDT`,
        side: String(row.metadata?.side || "").toUpperCase(),
        quantity: Math.max(0, finite(row.metadata?.maxQuantity)),
      }))
      .filter((row) => row.market !== "USDT" && ["LONG", "SHORT"].includes(row.side) && row.quantity > 0);
  } catch (error) {
    maintenanceErrors.push({
      stage: "V17_MANUAL_FUTURES_ALLOWANCE",
      error: error instanceof Error ? error.message : String(error),
    });
  }
