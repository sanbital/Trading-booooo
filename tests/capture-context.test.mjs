...
// Deployed v97 consumer contract (CAPTURE-CONTEXT-2-TRAJECTORY) as produced by the live
// public.doa_gpt_capture_context() RPC: twelve contiguous 5 s points ending at end_ms.
function trajectory(end,ingested=end){return {version:'CAPTURE-CONTEXT-2-TRAJECTORY',status:'AVAILABLE',buckets:12,start_ms:end-60000,end_ms:end,ingested_at_ms:ingested,
 trajectory:Array.from({length:12},(_,i)=>({end_ms:end-(11-i)*5000,d_mid_bps:i,d_spread_bps:0,d_ask_depth_25_pct:.01,d_bid_depth_25_pct:-.01,buy_share_5s:.3,d_buy_share:0,
  net_taker_quote_5s:-400,d_net_taker_quote:0,ask_book_net_5s:50,buy_impact_450_bps:1,d_buy_impact_bps:0,sell_impact_450_bps:1,d_sell_impact_bps:0}))};}
...
test('LIVE market reader carries authenticated capture through facts into the model input',async()=>{
 ...
 try{const {src}=await readSources('BTCUSDT',t,{fetchFn:async(url,opts)=>{
  if(url.startsWith('https://capture.test/')){reads++;assert.equal(opts.headers.apikey,'test-key');assert.equal(JSON.parse(opts.body).p_as_of,new Date(t).toISOString());return Response.json(trajectory(t-1000,t));}
