-- v7.1.8 permits protected targets and qualified rolling LOB reversals before 180 seconds.
-- The runtime deliberately executes both terminal LOB exits through the
...
begin
  ...
  -- Protected target exits are permitted throughout the first 180 seconds.
  -- The same protected-limit price and positive fee-net economics remain mandatory.
  ...
  if v_held_seconds < 180 then
    -- v7.1.8 computes the rolling 30s/50s confirmation windows from entry and
    -- persists the approved decision. v7.1.7 and legacy rows retain their previous
    -- 60-second minimum before a reversal exit can be approved.
    if v_policy_revision = '7.1.8-PRE180-TARGET-REVERSAL'
       and v_purpose = 'STOP'
  ...
  if v_policy_revision in ('7.1.7-FINAL-EXIT-POLICY', '7.1.8-PRE180-TARGET-REVERSAL')
...
comment on function public.guard_lob_sell_order_v714() is
  'v7.1.8 LOB sell guard: protected targets and qualified reversals are allowed before 180 seconds; terminal economics remain unchanged.';
