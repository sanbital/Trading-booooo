...
-- v7.5.2: keep the first 50% tranche at +5% / -4%.
-- Once only the residual 50% remains, exit it in full at +10% net or -4% net.
-- Trigger names/functions stay compatible with v7.5.1 so the authority switch is atomic.
...
comment on function public.enforce_residual_exit_position_policy_v751() is
  'v7.5.2 authority: first 50% TP5/SL4, residual 50% full exit at net TP10/SL4';
comment on function public.guard_residual_sell_order_v751() is
  'v7.5.2 sell guard: first tranche gross TP5/SL4, residual tranche net TP10/SL4';

-- Re-stamp active positions immediately so dashboard/state metadata reflects the new policy.
