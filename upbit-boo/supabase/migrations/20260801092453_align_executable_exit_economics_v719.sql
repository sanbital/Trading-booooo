...
-- v7.1.9 makes the runtime decision and database order guard evaluate the
-- exact executable quantity. Exchange step-size dust keeps its proportional
-- share of unrecovered cost instead of being charged to the sellable tranche.
...
begin
  ...
  if v_purpose in ('TARGET_1', 'TARGET_2') then
    if v_order_type = 'MARKET' or v_order_type not like 'LIMIT%' then
      raise exception using errcode='23514', message=format(
        'V719_TARGET_REQUIRES_PROTECTED_LIMIT market=%s purpose=%s order_type=%s',
        new.market, v_purpose, v_order_type
      );
    end if;
    if v_qty <= 0 or v_requested_price <= 0 then
      raise exception using errcode='23514', message=format(
        'V719_TARGET_MISSING_PRICE_OR_QUANTITY market=%s purpose=%s price=%s qty=%s',
        new.market, v_purpose, v_requested_price, v_qty
      );
    end if;
    if coalesce(v_target, 0) > 0 and v_requested_price < v_target then
      raise exception using errcode='23514', message=format(
        'V719_TARGET_PRICE_BELOW_PLAN market=%s requested=%s target=%s',
    ...
    if v_guarded_net <= v_min_profit_quote then
      raise exception using errcode='23514', message=format(
        'V719_TARGET_NET_NOT_POSITIVE market=%s purpose=%s protected_net=%s required=%s price=%s qty=%s allocated_cost=%s remaining_qty=%s',
  ...
  if coalesce(v_entry, 0) <= 0 or v_qty <= 0 or coalesce(v_quote_price, 0) <= 0 then
    raise exception using errcode='23514', message=format(
      'V719_EXIT_BLOCK_MISSING_ECONOMICS market=%s purpose=%s held=%s',
      new.market, v_purpose, round(v_held_seconds, 3)
    );
  end if;
  if v_quote_at is null or now() - v_quote_at > interval '30 seconds' then
    raise exception using errcode='23514', message=format(
      'V719_EXIT_BLOCK_STALE_QUOTE market=%s purpose=%s held=%s',
      new.market, v_purpose, round(v_held_seconds, 3)
    );
  end if;
  ...
  if v_held_seconds < 180 then
    ...
    raise exception using errcode='23514', message=format(
      'V719_PRE180_EXIT_BLOCKED market=%s purpose=%s held=%s reason=%s net=%s age=%s required=%s',
  ...
  if v_policy_revision in (
    ...
    raise exception using errcode='23514', message=format(
      'V719_POSITIVE_NET_EXIT_MISMATCH market=%s purpose=%s guarded_net=%s qty=%s allocated_cost=%s',
  ...
  if v_policy_revision in (
    ...
    raise exception using errcode='23514', message=format(
      'V719_MINUS3_EXIT_MISMATCH market=%s purpose=%s guarded_net_return_pct=%s',
  ...
  if v_guarded_net > v_min_profit_quote then
    raise exception using errcode='23514', message=format(
      'V719_POST180_PROFIT_MUST_USE_APPROVED_PATH market=%s purpose=%s guarded_net=%s',
  ...
  if v_drawdown_pct <= -3 then
    if v_purpose <> 'STOP' then
      raise exception using errcode='23514', message=format(
        'V719_MINUS3_EXIT_MUST_BE_STOP market=%s purpose=%s drawdown_pct=%s',
  ...
  raise exception using errcode='23514', message=format(
    'V719_POST180_HOLD market=%s purpose=%s held=%s guarded_net=%s drawdown_pct=%s',
    new.market, v_purpose, round(v_held_seconds, 3),
...
comment on function public.guard_lob_sell_order_v714() is
  'v7.1.9 LOB sell guard: executable quantity, allocated cost, fee-net and return thresholds match the runtime calculation.';
