-- v7.3.5 -- admit the runtime revision that carries its approval through the order-time re-quote.
--
-- The guard does not re-derive an exit decision. It reads the decision out of
-- exit_policy_quote and authorises a post-180 profit exit only when that object says
-- approved_reason = 'POSITIVE_NET_AFTER_180S'. The monitor cycle wrote that field, and the
-- order-time re-quote then replaced the whole object with an audit that never carried it,
-- so the guard saw an unapproved sell holding a profit and raised
-- V725_POST180_PROFIT_MUST_USE_APPROVED_PATH on every one of them. The -2 percent branch was
-- unaffected -- it does not re-quote -- so a position could leave at a loss and not at a
-- profit, for as long as it took to become one.
--
-- The repair is in the runtime, which now restates the approval and the executable price it
-- was approved at. This migration only admits the revision string that repair ships under.
-- The policy is unchanged from v7.3.4.
...
begin
  ...
  if v_held_seconds < 180 then
    if v_policy_revision in (
         ...
         '7.3.4-NET-POSITIVE-EXIT', '7.3.5-APPROVAL-SURVIVES-REQUOTE'
       )
       and v_purpose = 'STOP'
...
comment on function public.guard_lob_sell_order_v714() is
  'LOB sell guard v7.3.5: v7.3.4 policy, plus the 7.3.5 runtime revision. Before 180s, protected targets and qualified reversals only; after 180s, any positive executable net exits immediately. -2 percent of executable net forces a sell at any age. Emergency and reconciliation exits are unconditional.';
