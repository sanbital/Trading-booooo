-- v7.3.2 — the planned stop stops being an exit trigger.
--
-- 180 seconds is not a minimum hold. It selects which exit strategy applies: before it a
-- position leaves at its (fluid) target or on a confirmed book breakdown; after it any
-- positive executable net closes it immediately. Taking profit is available throughout.
--
-- The scanner's planned stop fitted neither regime. It returned from this guard on touch at
-- any age, so positions were closing seconds after entry at whatever the noise-derived stop
-- happened to be — EPICUSDT closed 29 seconds in, at -38bp gross. That is the repeated-stop
-- failure mode the policy exists to prevent.
--
-- It is removed. The only price that forces a sell is -2% of executable net, in both
-- regimes, so the post-180-only branch becomes one age-independent branch. Emergency and
-- reconciliation exits keep an unconditional escape because they are not price decisions.
-- Everything else -- pre-180 targets and qualified reversals, positive net after 180, the
-- bounded hold timeout, the multi-field quote resolution -- is unchanged.
...
begin
  ...
  if v_policy_revision = '7.3.0-EXECUTABLE-STOP-PARITY', '7.3.2-NO-PLANNED-STOP-TRIGGER'
...
comment on function public.guard_lob_sell_order_v714() is
  'LOB sell guard v7.3.2: no planned-stop exit trigger. Before 180s, protected targets and qualified reversals only; after 180s, positive executable net or the bounded hold timeout. -2% of executable net forces a sell at any age. Emergency and reconciliation exits are unconditional.';
