-- v7.3.4 -- the post-180 hold has no time cap.
--
-- After 180 seconds the rule is one line: any positive executable net closes the position
-- immediately, and a non-positive one holds until minus 2 percent. A bounded wait was added
-- from the 72h sample, where the losing tail ran to a 4,479s mean hold. That reading stands,
-- but it is not the policy, and a cap that closes a position the policy would still hold is
-- the wrong kind of safety. Removed.
--
-- What makes the wait tolerable is that "positive net" now means what it says: the runtime
-- stopped subtracting a slippage reserve from the number it compares against zero, so a
-- position showing a profit is taken instead of waiting out a margin it was never meant to
-- clear. Nothing else in the guard changes.
...
begin
  ...
  if v_held_seconds < 180 then
    if v_policy_revision in (
         ...
         '7.3.0-EXECUTABLE-STOP-PARITY', '7.3.2-NO-PLANNED-STOP-TRIGGER', '7.3.4-NET-POSITIVE-EXIT'
       )
       and v_purpose = 'STOP'
...
comment on function public.guard_lob_sell_order_v714() is
  'LOB sell guard v7.3.4: no planned-stop exit trigger and no post-180 time cap. Before 180s, protected targets and qualified reversals only; after 180s, any positive executable net exits immediately. -2 percent of executable net forces a sell at any age. Emergency and reconciliation exits are unconditional.';
