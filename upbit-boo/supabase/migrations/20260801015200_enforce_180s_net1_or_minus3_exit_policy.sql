...
create or replace function public.enforce_v713_net1_minus3_position_policy()
returns trigger
language plpgsql
...
begin
  ...
  new.metadata := coalesce(new.metadata, '{}'::jsonb) || jsonb_build_object(
    'exit_policy_revision', '7.1.3-180S-NET1-OR-MINUS3',
    'pre_180_exit_rule', 'TARGET_ONLY_AND_GUARDED_NET_QUOTE_GTE_1',
    'post_180_exit_rule', 'GUARDED_NET_QUOTE_GTE_1_OR_DRAWDOWN_LTE_MINUS_3PCT',
    'hard_stop_return_pct', -3,
    'minimum_exit_net_quote', 1,
    'target_shortfall_safety_bps', v_shortfall_bps,
    'timeout_loss_exit_enabled', false,
    'trail_exit_enabled', false,
    'lob_invalidation_exit_enabled', false,
...
create trigger zz_trading_positions_exit_policy_v713
before insert or update on public.trading_positions
for each row execute function public.enforce_v713_net1_minus3_position_policy();

create or replace function public.guard_lob_sell_order_v713()
returns trigger
language plpgsql
set search_path to 'public'
as $function$
...
begin
  ...
  if coalesce(v_entry, 0) <= 0 or v_qty <= 0 or coalesce(v_quote_price, 0) <= 0 then
    raise exception using errcode='23514', message=format(
      'V713_EXIT_BLOCK_MISSING_ECONOMICS market=%s purpose=%s held=%s',
      new.market, v_purpose, round(v_held_seconds, 3)
    );
  end if;
  if v_quote_at is null or now() - v_quote_at > interval '30 seconds' then
    raise exception using errcode='23514', message=format(
      'V713_EXIT_BLOCK_STALE_QUOTE market=%s purpose=%s held=%s',
      new.market, v_purpose, round(v_held_seconds, 3)
    );
  end if;
  ...
  v_guarded_net := v_guarded_fill_price * v_qty * (1 - v_fee_rate) - v_entry_cost;
  v_drawdown_pct := (v_quote_price / v_entry - 1) * 100;

  if v_held_seconds < 180 then
    if v_purpose not in ('TARGET_1', 'TARGET_2') or v_guarded_net < 1 then
      raise exception using errcode='23514', message=format(
        'V713_PRE180_EXIT_BLOCKED market=%s purpose=%s held=%s guarded_net=%s drawdown_pct=%s',
  ...
  if v_guarded_net >= 1 then
    if v_purpose not in ('TARGET_1', 'TARGET_2') then
      raise exception using errcode='23514', message=format(
        'V713_POST180_PROFIT_MUST_BE_TARGET market=%s purpose=%s guarded_net=%s',
  ...
  if v_drawdown_pct <= -3 then
    if v_purpose <> 'STOP' then
      raise exception using errcode='23514', message=format(
        'V713_MINUS3_EXIT_MUST_BE_STOP market=%s purpose=%s drawdown_pct=%s',
  ...
  raise exception using errcode='23514', message=format(
    'V713_POST180_HOLD market=%s purpose=%s held=%s guarded_net=%s drawdown_pct=%s',
    new.market, v_purpose, round(v_held_seconds, 3), round(v_guarded_net, 8), round(v_drawdown_pct, 6)
  );
end;
$function$;
...
create trigger zz_trading_orders_lob_exit_guard_v713
before insert on public.trading_orders
for each row execute function public.guard_lob_sell_order_v713();
