(() => {
  ...
  function buildCandidateReport(candidate, result) {
    ...
    const lines = [
      ...
      `- 외부 이벤트 판정: ${candidate.event_risk?.label || "데이터 없음"} / 상태 ${candidate.event_risk?.status || "UNKNOWN"} / 거래대금 이상 ${candidate.event_risk?.volume_anomaly ? "YES" : "NO"}`,
      `- 이벤트 공급자: ${Object.entries(candidate.event_risk?.provider_status || {}).map(([key, value]) => `${key}=${value}`).join(" / ") || "미설정"}`,
      ...(candidate.event_risk?.evidence || []).map(item => `- 이벤트 근거: [${item.severity}] ${item.category} / ${item.title} / ${item.displayed_date || item.event_date || item.published_at || "시점 미상"} / ${item.source}`),
