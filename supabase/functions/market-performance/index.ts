...
async function page(path: string, from: number, to: number): Promise<any[]> {
  ...
  if (!r.ok) {
    throw new Error(
      `database ${r.status}: ${typeof data === "string" ? data : JSON.stringify(data)}`,
    );
  }
...
async function limited(path: string): Promise<any[]> {
  return page(path, 0, PAGE_SIZE - 1);
}
...
function sum(rows: JsonRecord[], key: string): number {
  return rows.reduce((s, r) => s + Math.max(0, num(r[key])), 0);
}
...
function latestSnapshots(rows: JsonRecord[]): Record<Exchange, JsonRecord | null> {
  ...
  for (const row of rows) {
    ...
    if ((e === "upbit" || e === "binance" || e === "binance_futures") && !result[e]) {
      result[e] = row;
    }
  }
  return result;
...
function baseAsset(position: JsonRecord): string {
  ...
  if (
    (position.exchange === "binance" || position.exchange === "binance_futures") &&
...
function snapshotBalance(snapshot: JsonRecord | null, asset: string): number | null {
  ...
  if (Array.isArray(raw)) {
    const row = raw.find((v: any) =>
      String(v?.asset ?? v?.currency ?? "").toUpperCase() === target
    );
    if (!row) return 0;
  ...
  if (typeof raw === "object") {
    ...
    if (typeof value === "object") {
      return Math.max(0, num(value.free ?? value.balance)) + Math.max(0, num(value.locked));
    }
...
function buildUpbitRows(
  ...
  let soldQty = 0;
  const populated = sellOrders.map((order) => ({
    order,
    fills: fillsByOrder.get(String(order.id || "")) || [],
  }))
  ...
  for (let idx = 0; idx < populated.length; idx++) {
    ...
    rows.push({
      ...
      position_closed: String(position.state).toUpperCase() === "CLOSED",
      is_open: false,
      is_closed: true,
      immutable: String(position.state).toUpperCase() === "CLOSED",
      entry_at: entryAt,
      exit_at: exitAt,
      ...
      position_exit_index: idx + 1,
      position_exit_count: populated.length,
      accounting_quality: "TRADING_FILL_VERIFIED",
      pnl_source: "TRADING_FILLS",
    });
  }
  ...
  if (remaining > 0 && String(position.state).toUpperCase() !== "CLOSED") {
    ...
    if (qty > 0) {
      ...
      rows.push({
        ...
        entry_at: entryAt,
        exit_at: null,
        duration_seconds: duration(entryAt, new Date().toISOString()),
        quantity: qty,
        average_entry_price: avgEntry,
        ...
        exit_funds_quote: 0,
        invested_cost_quote: cost,
        current_value_quote: qty * mark,
        total_fees_quote: fee,
        net_pnl_quote: qty * mark - cost - fee,
        return_pct: cost > 0 ? (qty * mark - cost - fee) / cost * 100 : 0,
        accounting_quality: held == null
          ? "TRADING_FILL_LEDGER_ONLY"
          : "TRADING_FILL_PLUS_BALANCE_VERIFIED",
...
function groupExitFills(fills: JsonRecord[]): JsonRecord[][] {
  ...
  return [...groups.values()].sort((a, b) =>
    Date.parse(String(latest(a) || 0)) - Date.parse(String(latest(b) || 0))
  );
}

function buildBinanceRows(
  position: JsonRecord,
  fills: JsonRecord[],
  snapshot: JsonRecord | null,
): { rows: JsonRecord[]; quality: string } {
  ...
  for (let idx = 0; idx < groups.length; idx++) {
    ...
    rows.push({
      ...
      net_pnl_quote: pnl,
      return_pct: venue === "binance_futures" && cost > 0
        ? pnl / (cost / leverage) * 100
        : allocation.returnPct,
      ...
      position_exit_index: idx + 1,
      position_exit_count: groups.length,
      ...
      fee_accounting_quality: group.every((f) => f.fee_quote_amount != null)
        ? "EXCHANGE_RECORDED_FEE"
        : "FEE_PARTIAL",
...
function aggregate(
  ...
  const xs = rows.filter((r) => r.exchange === exchange),
    realisedRows = xs.filter((r) => r.row_type === "REALIZED_EXIT"),
    open = xs.filter((r) => r.is_open);
  const s = (a: JsonRecord[], k: string) => a.reduce((z, r) => z + num(r[k]), 0);
  const realised = s(realisedRows, "net_pnl_quote"),
    unrealised = s(open, "net_pnl_quote"),
    total = realised + unrealised;
  const invested = s(realisedRows, "invested_cost_quote") + s(open, "invested_cost_quote"),
    investedOpen = s(open, "invested_cost_quote");
  ...
  for (const row of realisedRows.filter((r) => r.position_closed)) {
    ...
    prev.pnl += num(row.net_pnl_quote);
    prev.cost += Math.max(0, num(row.invested_cost_quote));
    closedPositions.set(key, prev);
  }
  const closed = [...closedPositions.values()],
    wins = closed.filter((r) => r.pnl > 0),
    losses = closed.filter((r) => r.pnl < 0);
  const gp = wins.reduce((z, r) => z + r.pnl, 0),
    gl = Math.abs(losses.reduce((z, r) => z + r.pnl, 0));
  ...
  return {
    ...
    today_return_pct: s(todayRows, "invested_cost_quote") > 0
      ? s(todayRows, "net_pnl_quote") / s(todayRows, "invested_cost_quote") * 100
      : 0,
    total_fees_quote: s(xs, "total_fees_quote"),
    realized_exit_count: realisedRows.length,
    ...
    average_return_pct: closed.length
      ? closed.reduce((z, r) => z + (r.cost > 0 ? r.pnl / r.cost * 100 : 0), 0) / closed.length
      : 0,
    return_basis: "ACTUAL_EXIT_COST_WEIGHTED",
    source_of_truth: exchange === "binance" ? "EXCHANGE_TRADE_FILLS" : "TRADING_FILLS",
  };
}

function aggregateBinanceCombined(
  rows: JsonRecord[],
  spotSnapshot: JsonRecord | null,
  futuresSnapshot: JsonRecord | null,
): JsonRecord {
  const xs = rows.filter((r) => r.exchange === "binance" || r.exchange === "binance_futures");
  const realisedRows = xs.filter((r) => r.row_type === "REALIZED_EXIT"),
    open = xs.filter((r) => r.is_open);
  const s = (a: JsonRecord[], k: string) => a.reduce((z, r) => z + num(r[k]), 0);
  const realised = s(realisedRows, "net_pnl_quote"),
    unrealised = s(open, "net_pnl_quote"),
  ...
  const gp = wins.reduce((z, r) => z + r.pnl, 0),
    gl = Math.abs(losses.reduce((z, r) => z + r.pnl, 0));
  const today = kstDay(new Date().toISOString()),
    todayRows = realisedRows.filter((r) => kstDay(r.exit_at) === today);
  return {
    exchange: "binance",
    venues: ["binance", "binance_futures"],
    quote_currency: "USDT",
    current_equity_quote: num(spotSnapshot?.total_equity_quote) +
      num(futuresSnapshot?.total_equity_quote),
    managed_capital_quote: num(spotSnapshot?.managed_capital_quote) +
      num(futuresSnapshot?.managed_capital_quote),
    realized_net_pnl_quote: realised,
    open_net_pnl_quote: unrealised,
    cumulative_net_pnl_quote: total,
    cumulative_return_pct: invested > 0 ? total / invested * 100 : 0,
    ...
    average_return_pct: closed.length
      ? closed.reduce((z, r) => z + (r.cost > 0 ? r.pnl / r.cost * 100 : 0), 0) / closed.length
      : 0,
    return_basis: "VENUE_AWARE_ACTUAL_EXIT_MARGIN_WEIGHTED",
    source_of_truth: "BINANCE_SPOT_AND_FUTURES_EXCHANGE_TRADE_FILLS",
  };
}
...
Deno.serve(async (req: Request) => {
  ...
  if (!force && cache && now - cache.at < CACHE_TTL_MS) {
    return out({
      ...cache.body,
      cache_status: "HIT",
      cache_age_seconds: Math.floor((now - cache.at) / 1000),
    });
  }
  ...
  try {
    const [positions, orders, botFills, exchangeFills, snapshots] = await Promise.all([
      ...
      all(
        "trading_orders?select=id,position_id,exchange,side,purpose,paid_fee_quote,requested_at&order=requested_at.asc,id.asc",
        50000,
      ),
      all(
        "trading_fills?select=id,order_id,trade_id,price,volume,funds_quote,fee_quote_estimate,executed_at&order=executed_at.asc,id.asc",
        100000,
      ),
      all(
        "exchange_trade_fills?exchange=in.(binance,binance_futures)&position_id=not.is.null&select=id,position_id,side,exchange_trade_id,exchange_order_id,bot_order_id,price,quantity,quote_amount,base_asset,fee_asset,fee_amount,fee_quote_amount,realized_cost_quote,realized_proceeds_quote,realized_pnl_quote,accounting_status,source,executed_at&order=executed_at.asc,exchange_trade_id.asc",
        150000,
      ),
      limited(
    ...
    const latestSnap = latestSnapshots(snapshots),
      ordersByPosition = groupBy(orders, "position_id"),
      botFillsByOrder = groupBy(botFills, "order_id"),
      exchangeFillsByPosition = groupBy(exchangeFills, "position_id");
    const trades: JsonRecord[] = [], quality: Record<string, number> = {};
    for (const p of positions) {
      const exchange = String(p.exchange) as Exchange;
      const result = (exchange === "binance" || exchange === "binance_futures")
        ? buildBinanceRows(
          p,
          exchangeFillsByPosition.get(String(p.id)) || [],
          exchange === "binance_futures" ? latestSnap.binance_futures : latestSnap.binance,
        )
        : buildUpbitRows(
          p,
          ordersByPosition.get(String(p.id)) || [],
          botFillsByOrder,
          latestSnap.upbit,
        );
    ...
    trades.sort((a, b) =>
      new Date(b.exit_at || b.entry_at || 0).getTime() -
      new Date(a.exit_at || a.entry_at || 0).getTime()
    );
    const resultBody = {
      ok: true,
      version: VERSION,
      ...
      source_of_truth: {
        binance: "spot + futures exchange_trade_fills direct settlement",
        binance_spot: "exchange_trade_fills",
        binance_futures: "exchange_trade_fills / fapi userTrades",
        upbit: "trading_fills",
      },
      source_counts: {
        positions: positions.length,
        orders: orders.length,
        trading_fills: botFills.length,
        exchange_trade_fills: exchangeFills.length,
        performance_rows: trades.length,
      },
      account_reconciliation: {
        balance_snapshot_at: {
          upbit: latestSnap.upbit?.captured_at || null,
          binance: latestSnap.binance?.captured_at || null,
          binance_futures: latestSnap.binance_futures?.captured_at || null,
        },
        rule:
          "balances may verify/cap open quantity only; CLOSED economics never use current price or current balance",
      },
      exchanges: {
