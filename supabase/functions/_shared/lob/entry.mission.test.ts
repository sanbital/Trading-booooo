...
Deno.test("a single breakout score cannot declare LONG against tape, microprice and book", () => {
  ...
  assert(decision.reasons.includes("NO_PRIMARY_LOB_PATTERN"));
...
Deno.test("coherent breakout uses short-horizon target geometry", () => {
  ...
  assert(decision.targetBps >= 20);
  assert(decision.targetBps <= 60);
  assert(decision.targetBps < 75, "the old live average target was not horizon-executable");
...
Deno.test("busy tape does not create bullish probability when direction reverses", () => {
  ...
  const bearish = evaluateLobEntry(features({
    marketHeatScore: 100,
    tradeArrivalRate: 30,
    bookUpdateRate: 40,
