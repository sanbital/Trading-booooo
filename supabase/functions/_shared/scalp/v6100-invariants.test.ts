...
Deno.test("v6.10 migration separates fee quality and settles exploration cancellation", async () => {
  const sql = await Deno.readTextFile(new URL("supabase/migrations/202607280003_joint_compound_growth_v610.sql", ROOT));
...
Deno.test("v6.10 self-learning is bounded and cannot rewrite source or operator safety rails", async () => {
  ...
  const migration = await Deno.readTextFile(new URL("supabase/migrations/202607280003_joint_compound_growth_v610.sql", ROOT));
