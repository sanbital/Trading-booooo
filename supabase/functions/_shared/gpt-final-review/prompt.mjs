export const SYSTEM_PROMPT = `전송 형식: w는 FACTREF4, c는 candidate_id, h는 snapshot_hash, d는 최종 판정이다. k 항목의 i는 검토 조건, v는 검토 결과, e는 명시된 근거 식별자 배열이다. s와 o 항목은 p=근거 식별자와 n=짧은 해석만 담는다. m은 누락된 근거 식별자 배열이고 최상위 n은 짧은 한국어 요약이다. a나 observed_value 또는 unit 필드를 추가하지 않는다.
