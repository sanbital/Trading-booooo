...
function parseKline(row: unknown): LaneBar {
  ...
  if (
    !Number.isFinite(
      parsed.openTime + parsed.open + parsed.high + parsed.low + parsed.close + parsed.quoteVolume,
    )
  ) {
...
async function fetchClosedBars(symbol: string, signalOpenTime: number): Promise<LaneBar[]> {
  ...
  if (bars[bars.length - 1].openTime !== signalOpenTime) {
    throw new Error(
      `LATEST_COMPLETED_BAR_MISSING:${symbol}:${bars[bars.length - 1].openTime}:${signalOpenTime}`,
    );
  }
...
Deno.serve(async (request) => {
  ...
  if (!supabaseUrl || !serviceRole) {
    return response(503, { ok: false, invocationId, error: "SUPABASE_RUNTIME_SECRET_MISSING" });
  }
  ...
  try {
    ...
    if (!diagnosticOnly && now - completedAt > MAX_DATA_AGE_MS) {
      return response(409, {
        ok: false,
        invocationId,
        error: "INVOCATION_OUTSIDE_ENTRY_WINDOW",
        signalOpenTime,
        ageMs: now - completedAt,
      });
    }
    ...
    const rows = await mapLimit(
      symbols,
      4,
      async (symbol) => [symbol, await fetchClosedBars(symbol, signalOpenTime)] as const,
    );
    ...
    const route = evaluated[0]?.lane ??
      (btcContext.btc72 < -0.05
        ? "BEAR"
        : btcContext.btc72 <= 0.04
        ? "RANGE"
        : btcContext.btc72 > 0.05
        ? "BULL"
        : "CASH");
    ...
    if (!registered.has(config.fingerprint)) {
      return response(200, {
        ...
        route,
        inserted: 0,
        blocked: "STRATEGY_NOT_VALIDATED_OR_REGISTERED",
    ...
    if (flag.live_enabled && Deno.env.get("V10_LANE_EXECUTION_BRIDGE_ENABLED") !== "true") {
      return response(503, {
        ...
        invocationId,
        route,
        error: "LIVE_FLAG_WITHOUT_EXECUTION_BRIDGE",
      });
    }
    ...
    const active = recentSignals
      .filter((row) =>
        Date.parse(row.signal_bar_at) + Number(row.hold_hours) * 60 * 60 * 1000 > signalOpenTime
      )
      .map((row) => ({
        symbol: row.symbol,
        exitBarAt: Date.parse(row.signal_bar_at) + Number(row.hold_hours) * 60 * 60 * 1000,
      }));

    const cooldownEligible = evaluated.filter((decision) => {
      if (
        !decision.eligible || decision.lane !== route || !decision.fingerprint ||
        !decision.cooldownHours
      ) return false;
      const last = recentSignals
        .filter((row) =>
          row.fingerprint === decision.fingerprint && row.symbol === decision.features.symbol
        )
    ...
    const capacity = Math.min(
      MAX_CONCURRENT_TOTAL,
      Number(flag.max_concurrent ?? MAX_CONCURRENT_TOTAL),
    );
    ...
    return response(200, {
      ...
      expectedExits: admitted.map((decision) => ({
        symbol: decision.features.symbol,
        exitBarAt: expectedExitBarAt(signalOpenTime, decision.holdHours ?? 0),
      })),
      dataErrors,
    });
  } catch (error) {
