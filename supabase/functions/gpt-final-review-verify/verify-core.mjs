...
export async function faults({store,body}){
  ...
  await scenario('wrongCandidate',async(url,init)=>{const input=JSON.parse(JSON.parse(init.body).input[1].content);
    return json({id:'x',status:'completed',model:MODEL,service_tier:'default',output:[{type:'message',content:[{type:'output_text',text:JSON.stringify({w:input.w,c:'c_'+'0'.repeat(32),h:input.h,d:'PASS',k:[{i:'CURRENT_REACCELERATION',v:'SUPPORTED',e:['C_return_5m']}],s:['C_return_5m','C_last_body'],o:[],m:[],n:'근거가 유지됩니다'})}]}],usage:{input_tokens:10,output_tokens:5,input_tokens_details:{cached_tokens:0}}});});
  await scenario('wrongSnapshot',async(url,init)=>{const input=JSON.parse(JSON.parse(init.body).input[1].content);
    return json({id:'x',status:'completed',model:MODEL,service_tier:'default',output:[{type:'message',content:[{type:'output_text',text:JSON.stringify({w:input.w,c:input.c,h:'f'.repeat(64),d:'PASS',k:[{i:'CURRENT_REACCELERATION',v:'SUPPORTED',e:['C_return_5m']}],s:['C_return_5m','C_last_body'],o:[],m:[],n:'근거가 유지됩니다'})}]}],usage:{input_tokens:10,output_tokens:5,input_tokens_details:{cached_tokens:0}}});});
