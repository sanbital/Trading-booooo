...
const GW_SECRET = Deno.env.get("BINANCE_GATEWAY_SHARED_SECRET") ?? Deno.env.get("GATEWAY_SHARED_SECRET") ?? "";
...
async function sign(message: string) {
  if (!hmacKey) {
    hmacKey = await crypto.subtle.importKey("raw", enc.encode(GW_SECRET), { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
  }
...
async function gatewayOnce(command: Json) {
  ...
  try {
    ...
    if (!res.ok) throw new Error(`GATEWAY_${res.status}:${(typeof parsed === "string" ? parsed : JSON.stringify(parsed)).slice(0, 500)}`);
...
async function publicGet(path: string, params: Record<string, string | number>) {
  ...
  if (!res.ok) throw new Error(`BINANCE_PUBLIC_${res.status}:${(typeof parsed === "string" ? parsed : JSON.stringify(parsed)).slice(0, 300)}`);
  return parsed;
}

function assets(symbol: string) {
  return symbol.endsWith(QUOTE) ? { base: symbol.slice(0, -QUOTE.length), quote: QUOTE } : { base: symbol, quote: QUOTE };
}
...
function oldestFirst(stateMap: Map<string, SyncState>) {
  return (a: string, b: string) => (stateMap.get(a)?.lastSynced ?? 0) - (stateMap.get(b)?.lastSynced ?? 0) || a.localeCompare(b);
}
...
async function loadAllOrders(sb: ReturnType<typeof createClient>): Promise<BotOrder[]> {
  const rows: BotOrder[] = [];
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const acquired = await sb.rpc("acquire_exchange_trade_sync_lease", { p_owner: owner, p_ttl_seconds: 110 });
    ...
    const created = await sb.from("exchange_trade_sync_runs").insert({ exchange: EXCHANGE, account_scope: SCOPE }).select("id").single();
    runId = created.data?.id as string | undefined;
    if (!runId) return reply(500, { ok: false, error: "RUN_LOG_CREATE_FAILED", detail: created.error?.message ?? null });
    ...
    try {
      ...
      const [positionsRes, orders, statesRes, profilesRes] = await Promise.all([
        sb.from("trading_positions").select("market").eq("exchange", EXCHANGE).eq("state", "OPEN").limit(500),
        loadAllOrders(sb),
        sb.from("exchange_trade_sync_state").select("market,last_exchange_trade_id,last_synced_at,last_trade_at").eq("exchange", EXCHANGE).eq("account_scope", SCOPE).limit(1000),
      ...
      for (const p of positionsRes.data ?? []) if (p.market) { urgent.add(String(p.market)); all.add(String(p.market)); }
      for (const b of account?.balances ?? []) {
        if (num(b.free) + num(b.locked) > 0 && b.asset !== QUOTE) { urgent.add(`${b.asset}${QUOTE}`); all.add(`${b.asset}${QUOTE}`); }
      }
      ...
      if (unsynced.length) {
        ...
        for (const m of candidates.filter((m) => urgent.has(m)).sort(oldestFirst(stateMap)).slice(0, URGENT_BACKFILL_SLOTS)) chosen.add(m);
        for (const m of unsynced.sort()) if (chosen.size < BACKFILL_BATCH) chosen.add(m);
        for (const m of candidates.sort(oldestFirst(stateMap))) if (chosen.size < BACKFILL_BATCH) chosen.add(m);
        markets = [...chosen];
      ...
      await sb.from("exchange_trade_sync_runs").update({
        ...
        details: { selection, candidate_markets: candidates.length, unsynced_markets: unsynced.length, source: "binance-static-ip-gateway" },
      }).eq("id", runId);

      for (const market of markets) {
        try {
          const state = stateMap.get(market);
          const command: Json = { exchange: EXCHANGE, action: "trade_history", market, limit: 1000 };
          ...
          for (const t of trades) {
            ...
            if (matched) automated++; else manual++;
            rows.push({
              exchange: EXCHANGE, account_scope: SCOPE, market,
              ...
              is_maker: t.isMaker, source,
              bot_order_id: matched?.id ?? null, position_id: matched?.position_id ?? null,
          ...
          if (rows.length) {
            ...
            const acc = await sb.rpc("rebuild_exchange_trade_accounting", {
              p_exchange: EXCHANGE, p_account_scope: SCOPE, p_market: market,
            });
          ...
          const recon = await sb.rpc("reconcile_positions_from_exchange_fills", {
            p_exchange: EXCHANGE, p_market: market,
          });
          ...
          fillsRelinked += num(reconData.fills_linked_to_orders) + num(reconData.fills_attributed_fifo);

          const st = await sb.from("exchange_trade_sync_state").upsert({
            exchange: EXCHANGE, account_scope: SCOPE, market,
            last_exchange_trade_id: maxId, last_trade_at: maxAt,
          ...
          await sb.from("exchange_trade_sync_state").upsert({
            exchange: EXCHANGE, account_scope: SCOPE, market,
            last_synced_at: new Date().toISOString(),
            last_status: "ERROR", last_error: message, updated_at: new Date().toISOString(),
          }, { onConflict: "exchange,account_scope,market" });
        }
      }
      ...
      let refreshed = 0;
      if (upserted > 0) {
        const refresh = await sb.rpc("refresh_real_trade_scorecards", { p_exchange: EXCHANGE, p_lookback: "8 days" });
        if (refresh.error) errors.push({ market: "__SCORECARD__", error: refresh.error.message.slice(0, 500) });
      ...
      await sb.from("exchange_trade_sync_runs").update({
        ...
        status, markets_succeeded: succeeded, fills_seen: seen, fills_upserted: upserted,
        automated_fills: automated, manual_fills: manual, unmatched_fills: unmatched,
        error_message: errors.length ? errors.slice(0, 5).map((e) => `${e.market}:${e.error}`).join(" | ") : null,
        details: {
          errors: errors.slice(0, 25), scorecards_refreshed: refreshed, selection,
          candidate_markets: candidates.length, unsynced_markets: unsynced.length,
          orders_loaded: orders.length,
          positions_closed: positionsClosed, fills_relinked: fillsRelinked,
          source: "binance-static-ip-gateway",
        },
      }).eq("id", runId);

      return reply(status === "ERROR" ? 500 : 200, {
        ok: status !== "ERROR", status, run_id: runId, selection,
        markets_requested: markets.length, markets_succeeded: succeeded,
        fills_seen: seen, fills_upserted: upserted,
        automated_fills: automated, manual_fills: manual, unmatched_fills: unmatched,
        orders_loaded: orders.length, positions_closed: positionsClosed, fills_relinked: fillsRelinked,
        scorecards_refreshed: refreshed, errors: errors.slice(0, 10),
      });
    } catch (e) {
      const message = e instanceof Error ? e.message : String(e);
      await sb.from("exchange_trade_sync_runs").update({
        completed_at: new Date().toISOString(), status: "ERROR",
        error_message: message.slice(0, 1000), details: { source: "binance-static-ip-gateway" },
      }).eq("id", runId);
