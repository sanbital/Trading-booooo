...
Deno.test("primary pullback breakout needs a real prior impulse", () => {
  const books = [
  ...
  const stage = { state:"BREAKOUT_RECLAIM", score:72, metrics:{ impulseReturn:.05, takerBuyShare:.60, qvRatio:2, upperWick:.1, closeLocation:.8, absorptionRisk:false, antiChaseAtr:.5 } };
  const d = microDecision(stage, .02, book, .03, 65, 8, 8);
  if (!d.entryReady) throw new Error("expected primary entry");
  const weakImpulse = microDecision({ ...stage, metrics:{ ...stage.metrics, impulseReturn:.005 } }, .02, book, .03, 65, 8, 8);
  if (weakImpulse.entryReady) throw new Error("weak impulse must not enter");
  const direct = microDecision({ ...stage, state:"DIRECT_BREAKOUT" }, .02, book, .03, 65, 8, 8);
