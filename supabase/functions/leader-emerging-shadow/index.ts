...
import {runFd1Audit} from './fd1-audit.mjs';
import {runFd1Early} from './fd1-early.mjs';
...
const MODES={scan:runScan,wait:runWait,outcome:runOutcome,diagnostic:runDiagnostic,
  ...
  health:runHealth,
  // Independent observation: reads production review packets and writes only shadow_le.
  fd1audit:runFd1Audit,
  fd1early:runFd1Early};
...
Deno.serve(async req=>{
  ...
  try{
    ...
    const out=await run({db,store,store2,guard,now:Date.now,apiKey});
