...
async function latestSnapshot(db: DbClient): Promise<any> {
  const { data, error } = await db.from("trading_account_snapshots")
    .select("captured_at,total_equity_quote,available_quote,positions,positions_complete,positions_revision")
...
async function executorPreflight(db: DbClient, symbol = "BTCUSDT"): Promise<JsonRecord> {
  const [{ data: flag, error: flagError }, snapshot] = await Promise.all([
    db.from("v10_lane_flags")
      .select("lane,validated,shadow_enabled,live_enabled,notional_usdt,leverage,engine_revision,spec_sha256,max_concurrent,max_aggregate_notional_usdt")
...
async function executeClaimedEntry(db: DbClient, signal: ClaimedSignal): Promise<JsonRecord> {
  if (signal.lane !== "RANGE" || signal.side !== "LONG") {
    await db.from("v10_lane_claims").update({
      claim_state: "REJECTED", reject_reason: "V10_EXECUTOR_RANGE_LONG_ONLY",
      updated_at: new Date().toISOString(),
    }).eq("signal_id", signal.signal_id);
  ...
  const { data: p10SameMarket } = await db.from("trading_positions")
    ...
    .in("state", ["ENTRY_PENDING", "OPEN", "EXITING", "RECONCILING",
      "RECONCILIATION_FAILED", "MANUAL_INTERVENTION_REQUIRED"]).limit(1);
  if (p10SameMarket?.length) {
    await db.from("v10_lane_claims").update({
      claim_state: "REJECTED", reject_reason: "P10_MARKET_ALREADY_TRACKED",
      updated_at: new Date().toISOString(),
    }).eq("signal_id", signal.signal_id);
  ...
  try {
    ...
    if (fill.executedQuantity > 0) {
      ...
      return {
        entered: true, signalId: signal.signal_id, symbol: signal.symbol,
        positionId: position.id, quantity: fill.executedQuantity,
        entryPrice: fill.averagePrice, marginUsdt: margin, leverage,
      };
    }
    if (isTerminalNoFill(fill)) {
      await db.from("v10_lane_order_intents").update({
        state: "REJECTED", reject_reason: `TERMINAL_NO_FILL:${fill.status}`,
        updated_at: new Date().toISOString(),
      }).eq("id", intent.id);
      await db.from("v10_lane_claims").update({
        claim_state: "REJECTED", reject_reason: `TERMINAL_NO_FILL:${fill.status}`,
        updated_at: new Date().toISOString(),
      }).eq("signal_id", signal.signal_id);
...
async function evaluateLiveExits(db: DbClient): Promise<JsonRecord> {
  ...
  if (!positions?.length) return { evaluatedPositions: 0, decisions: 0 };
  ...
  return { evaluatedPositions: positions.length, decisions };
}
...
async function finalizeExitFill(
  ...
  const remainingBefore = Math.max(0, finite(position.remaining_quantity, finite(position.quantity)));
...
async function processExitDecisions(db: DbClient): Promise<JsonRecord[]> {
  const { data: decisions, error } = await db.from("v10_lane_exit_decisions")
    ...
    .in("action", [
      "PARTIAL_AT_TRIGGER", "FULL_AT_TRIGGER", "PARTIAL_NEXT_OPEN", "FULL_NEXT_OPEN",
    ])
  ...
  for (const decision of decisions || []) {
    ...
    const quantity = floorToStep(Math.min(remaining, remaining * finite(decision.fraction, 1)), step);
    if (!(quantity > 0)) {
      await openCircuit(db, `V10_EXIT_ZERO_QUANTITY:${decision.id}`);
      continue;
    }
    const clientOrderId = v10ClientOrderId(
      "v10x", `${decision.id}${String(position.id).replaceAll("-", "")}`,
    );
    const requestPayload = {
      action: "create_order",
      order: {
...
async function runExecutor(db: DbClient): Promise<JsonRecord> {
  ...
  const exitEvaluation = await evaluateLiveExits(db);
