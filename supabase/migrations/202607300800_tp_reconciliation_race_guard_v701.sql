...
--   4) exclude contaminated outcomes from all realized-edge learning.
...
with affected as (
  ...
  join public.trading_events e
    on e.position_id=p.id and e.code='MANUAL_POSITION_REDUCTION'
  join public.trading_orders o
    on o.position_id=p.id and o.purpose='TARGET_1' and o.executed_volume>0
...
select public.record_asset_lock_check_v610(
  exchange,base_asset,'MISMATCH',
  'UNTRACKED_RESIDUAL_AFTER_TP_RECONCILIATION_RACE',
  jsonb_build_object('position_id',id,'market',market,'revision','7.0.1-TP-RECONCILIATION-SAFETY')
) from affected;

-- Synthetic assertions: the global engine is live, only resting TP is disabled, and contaminated
-- outcomes cannot enter the verified-profile cohorts.
do $$
declare cfg public.trading_settings%rowtype; n integer;
begin
  ...
  select count(*) into n from public.lob_online_outcomes
   where accounting_quality='TP_RECONCILIATION_RACE_INVALID'
     and position_id in (
       select p.id from public.trading_positions p
  ...
  if n=0 then raise exception 'V701_CONTAMINATED_OUTCOMES_NOT_QUARANTINED'; end if;
end $$;
