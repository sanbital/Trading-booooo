...
-- Restore the user-defined staged exit policy after the accidental full-stop hotfix:
-- first 50% at +5% or -4%, then the residual 50% at +10% or -4%.
-- The closed-position fill-ledger PnL repair from the preceding migration is retained.
...
begin
  ...
  new.metadata := coalesce(new.metadata, '{}'::jsonb) || jsonb_build_object(
    'exit_policy_revision', '7.5.2-RESTORED-SPLIT-SL4-R2',
...
comment on function public.enforce_residual_exit_position_policy_v751() is
  'v7.5.2 restored policy: first 50% TP5/SL4, residual 50% full exit at net TP10/SL4';
comment on function public.guard_residual_sell_order_v751() is
  'v7.5.2 restored sell guard: first tranche gross TP5/SL4, residual tranche net TP10/SL4';

-- Fire the policy trigger so active positions immediately carry the restored metadata.
...
begin
  if position(
    ...
    raise exception 'FIRST_TRANCHE_HALF_SELL_GUARD_MISSING';
  end if;
  ...
  if position(
    ...
    raise exception 'ACCIDENTAL_FULL_STOP_GUARD_STILL_PRESENT';
  end if;

  if exists (
    select 1
    from public.trading_positions p
    where p.state in ('OPEN','EXITING')
      and coalesce(p.initial_quantity, 0) > 0
      and (
        p.metadata#>>'{half_hold_policy,first_stop_loss_pct}' is distinct from '-4'
        or p.metadata#>>'{half_hold_policy,residual_stop_loss_pct}' is distinct from '-4'
        or p.metadata#>'{half_hold_policy}' ? 'loss_exit_ratio'
      )
  ) then
    raise exception 'ACTIVE_POSITION_SPLIT_STOP_METADATA_INVALID';
  end if;
end;
$verify$;
