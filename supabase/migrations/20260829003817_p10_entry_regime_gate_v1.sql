create table if not exists public.p10_entry_regime_gate_attempts (
  ...
  unique (venue, market, signal_time, side, policy_revision)
);

alter table public.p10_entry_regime_gate_attempts enable row level security;
revoke all on public.p10_entry_regime_gate_attempts from anon, authenticated;
grant select, insert, update on public.p10_entry_regime_gate_attempts to service_role;
grant usage, select on sequence public.p10_entry_regime_gate_attempts_id_seq to service_role;
...
declare
  ...
  v_policy_revision constant text := 'P10-ENTRY-REGIME-GATE-v1';
begin
  begin
    ...
    if v_live_candidate and v_recommendation = 'BLOCK' then
      v_decision := 'BLOCK';
  ...
  insert into public.p10_entry_regime_gate_attempts (
    ...
    jsonb_build_object('gate',coalesce(v_gate,'{}'::jsonb),'recommendation',v_recommendation,'live_gate_candidate',v_live_candidate,'evidence',coalesce(p_evidence,'{}'::jsonb))
  )
