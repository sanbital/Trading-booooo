-- The 20260818083500 futures recovery state-machine migration recreates
-- guard_residual_sell_order_v751(). That recreation removes the v7.6.10 Late Recovery
-- owner inserted by 20260818120000. Re-install only the residual-guard branch, idempotently,
-- after the state-machine owner has been replayed.
...
BEGIN
  ...
  IF v_def IS NULL THEN
    RAISE EXCEPTION 'guard_residual_sell_order_v751() missing';
  END IF;

  IF position('LATE_RECOVERY_NET_POSITIVE_EXIT' IN v_def) > 0
     AND position('LATE_RECOVERY_DRAWDOWN_33_EXIT' IN v_def) > 0
     AND position('LATE_RECOVERY_OUTSIDE_OWNER_WINDOW' IN v_def) > 0 THEN
    RETURN;
  END IF;

  IF (length(v_def) - length(replace(v_def, v_anchor, ''))) / length(v_anchor) <> 1 THEN
    RAISE EXCEPTION 'LATE_RECOVERY_RESTORE_SOURCE_DRIFT';
  END IF;
...
COMMENT ON FUNCTION public.guard_residual_sell_order_v751() IS
  'Split/residual exit authority with fail-closed Late Recovery 460s/FOK guard restored after recovery state-machine replay.';
